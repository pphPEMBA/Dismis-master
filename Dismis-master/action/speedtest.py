...
def shell():
    ...
    speak('Retrieving speedtest.net configuration...')
    try:
        speedtest = Speedtest(
    ...
    printer('Testing from %(isp)s (%(ip)s)...' % speedtest.config['client'],
            quiet)
    speak('Testing from %(isp)s (%(ip)s)...' % speedtest.config['client'])

    if not args.mini:
        printer('Retrieving speedtest.net server list...', quiet)
        speak('Retrieving speedtest.net server list')
        try:
            ...
            speak('no matched servers')
        except (ServersRetrievalError,) + HTTP_ERRORS:
            printer('Cannot retrieve speedtest server list', error=True)
            speak('cannot retrieve speedtest server list')
            raise SpeedtestCLIError(get_exception())
        ...
        if args.server and len(args.server) == 1:
            ...
            speak('Selecting best server based on ping')
    ...
    speak('Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: '
    ...
    if args.download:
        ...
        printer('Download: %0.2f M%s/s' %
                ((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
        speak('Download: %0.2f M%s/s' %
                ((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]))
    else:
        printer('Skipping download test', quiet)
        speak('Skipping download speed')
    ...
    if args.upload:
        ...
        printer('Upload: %0.2f M%s/s' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
        speak('Upload: %0.2f M%s/s' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]))
    else:
        printer('Skipping upload test', quiet)
        speak('Skipping upload test')
